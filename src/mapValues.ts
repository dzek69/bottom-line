 ...
 */
const REMOVE = {};
type MapValuesFn = (value: unknown, key: string) => unknown | typeof REMOVE;
 ...
 */
const mapValues = (source: Source, fn: MapValuesFn): unknown => {
    const keys = Object.keys(source);

    return keys.reduce((result, key) => {
        const value = fn((source)[key], key);
        if (value !== REMOVE) {
            (result as Source)[key] = value; // eslint-disable-line no-param-reassign
        }
        return result;
